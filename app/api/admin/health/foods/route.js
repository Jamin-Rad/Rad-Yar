...
export async function DELETE(request) {
  ...
  const { error } = custom
    ? await supabaseAdmin.from('health_custom_foods').delete().eq('owner_id', ADMIN_HEALTH_OWNER).eq('id', id)
    : await supabaseAdmin.from('health_deleted_ids').upsert({ owner_id: ADMIN_HEALTH_OWNER, item_type: 'food', item_id: id })
...
export async function PATCH(request) {
  ...
  const { error } = await supabaseAdmin.from('health_deleted_ids')
    .delete().eq('owner_id', ADMIN_HEALTH_OWNER).eq('item_type', 'food').eq('item_id', id)
