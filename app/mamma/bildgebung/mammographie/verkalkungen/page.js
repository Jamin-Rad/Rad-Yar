  ...
  return <div className={caseStyles.assessment}>
    ...
    <div className={caseStyles.assessmentCalculator}>
      ...
      <div className={`${caseStyles.assessmentResult} ${caseStyles[CALC_CLASS[resultCategory]]}`} aria-live="polite">
        <span>Modellergebnis</span><strong>BI-RADS {resultCategory}</strong><b>{result.label}</b><small>Risikorahmen der Kategorie: {result.risk}</small><small>{resultCategory==='3'&&!(morph==='round'&&dist==='grouped')?'Klinische Kategorie gesondert prüfen: Dieses Modellergebnis begründet keine Verlaufskontrolle.':result.action}</small>
      </div>
    </div>
    ...
    <div className={caseStyles.contextPanel}>
      ...
      <div className={caseStyles.negativeMriNote} aria-live="polite"><SectionIcon id="ultraschall"/><div><h4>Negative MRT · BI-RADS {resultCategory}</h4><p>{result.mri}</p></div></div>
    </div>
  </div>
}
...
function GermanContent(){return <>
  <Section {...GERMAN_SECTIONS[0]} title={GERMAN_SECTIONS[0].label.de}>
    ...
    <div><h3 className={`${styles.takeTitle} ${caseStyles.sectionTitleRule}`}>Technik</h3><div className={caseStyles.techniqueGrid}><article><SectionIcon id="technik"/><h4>Mammographie · MG</h4><p>Gezielte 2D-Vergrößerungsaufnahmen zeigen die Kalkdetails.</p></article><article><SectionIcon id="ausdehnung"/><h4>Tomosynthese · DBT</h4><p>Ergänzt Lokalisation und Gewebekontext, ersetzt bei der Kalkabklärung aber keine Vergrößerungsaufnahmen.</p></article></div></div>
  </Section>

  <Section {...GERMAN_SECTIONS[1]} title={GERMAN_SECTIONS[1].label.de}>
    <p className={`${styles.lead} ${caseStyles.emphasizedLead}`}>Makroverkalkungen (&gt; 2 mm) beziehungsweise typisch grobschollige Verkalkungen (irregulär geformt, jedoch mit glatter Begrenzung) sind in der Regel benign.</p>
    ...
    <article className={caseStyles.caseStudy}>
      ...
      <div className={caseStyles.caseGallery}>
        <figure><a href="https://radiopaedia.org/cases/86372/studies/102403#t=im&v1i=54303996&v1z=1&v2i=54303997&v2z=1&v3i=54303998&v3z=1&v4i=54303999&v4z=1" target="_blank" rel="noreferrer"><Image src="/mamma/mammographie/verkalkungen/case-86372/r-cc.png" alt="CC-Mammographie rechts mit grobscholligen Verkalkungen" width={394} height={814}/></a><figcaption>CC rechts</figcaption></figure>
        <figure><a href="https://radiopaedia.org/cases/86372/studies/102403#t=im&v1i=54303996&v1z=1&v2i=54303997&v2z=1&v3i=54303998&v3z=1&v4i=54303999&v4z=1" target="_blank" rel="noreferrer"><Image src="/mamma/mammographie/verkalkungen/case-86372/r-mlo.png" alt="MLO-Mammographie rechts mit grobscholligen Verkalkungen" width={413} height={818}/></a><figcaption>MLO rechts</figcaption></figure>
      </div>
      <p className={caseStyles.caseDescription}><strong>Größe und Morphologie:</strong> Mehrere große, grobe und glatt begrenzte Verkalkungen in der rechten Brust.</p>
      <p className={caseStyles.caseCredit}>Bildbeispiel: <a href="https://radiopaedia.org/cases/86372/studies/102403#t=im&v1i=54303996&v1z=1&v2i=54303997&v2z=1&v3i=54303998&v3z=1&v4i=54303999&v4z=1" target="_blank" rel="noreferrer">Radiopaedia.org, Fall 86372 (Vollbild)</a>.</p>
    </article>
  ...
  <Section {...GERMAN_SECTIONS[2]} title={GERMAN_SECTIONS[2].label.de}>
    ...
    <p className={caseStyles.biradsCaption}>Einordnung: <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC5787219/" target="_blank" rel="noreferrer">BI-RADS 3: Current and Future Use</a>.</p>
  </Section>
  ...
  <Section {...GERMAN_SECTIONS[3]} title={GERMAN_SECTIONS[3].label.de}>
    ...
    <div className={`${styles.rule} ${caseStyles.multiParagraph}`}><strong>Merke</strong><ul className={caseStyles.rememberList}><li><b>Duktales Muster, keine sichere Histologie:</b> Suspekte Mikroverkalkungen in linearer oder segmentaler Verteilung sind häufig mit DCIS assoziiert. Auch ein invasives Karzinom mit intraduktaler Komponente ist möglich.</li><li><b>Linear ist nicht automatisch maligne:</b> Grobe, glatte Stäbchen können sekretorisch bedingt sein, etwa bei Duktektasie/Plasmazellmastitis. Feine, irreguläre lineare oder verzweigte Partikel sind dagegen suspekt.</li></ul></div>
  </Section>
  ...
  <Section {...GERMAN_SECTIONS[5]} title={GERMAN_SECTIONS[5].label.de}>
    ...
    <KalkAssessment/>
    <div className={caseStyles.warningSigns}><header><SectionIcon id="warnung"/><div><small>Gezielt beachten</small><h3>Warnzeichen auf einen Blick</h3></div></header><ul><li><SectionIcon id="morphologie"/><div><b>Suspekte Morphologie</b><p>Fein pleomorph oder fein linear/verzweigt; deutliche Form- und Größenheterogenität.</p></div></li><li><SectionIcon id="verteilung"/><div><b>Duktale Verteilung</b><p>Lineare oder segmentale Anordnung suspekter Partikel.</p></div></li><li><SectionIcon id="ausdehnung"/><div><b>Große Ausdehnung</b><p>Ein ausgedehntes Kalkareal im Gesamtbefund berücksichtigen.</p></div></li><li><SectionIcon id="kontext"/><div><b>Assoziierte Gewebeveränderung</b><p>Begleitende Masse oder Architekturstörung.</p></div></li></ul></div>
    <div className={`${styles.rule} ${caseStyles.multiParagraph}`}><strong>Stabilität ≠ sicher benign</strong><p>DCIS kann langsam wachsen und über Jahre bildmorphologisch unverändert bleiben. Stabilität entkräftet eine suspekte Morphologie daher nicht.</p></div>
    <p className={caseStyles.biradsCaption}>Zum natürlichen Verlauf: <a href="https://pmc.ncbi.nlm.nih.gov/articles/PMC4484537/" target="_blank" rel="noreferrer">Ductal Carcinoma In Situ of the Breast</a>.</p>
  </Section>
  ...
  <Section {...GERMAN_SECTIONS[6]} title={GERMAN_SECTIONS[6].label.de}>
    ...
    <article className={caseStyles.caseStudy}>
      ...
      <div className={caseStyles.caseGallery}>
        <figure><a href="https://radiopaedia.org/cases/159211/studies/130479?lang=us#t=im&v1i=60684407&v1z=1&v2i=60684408&v2z=1&v3i=60684410&v3z=1&v4i=60684413&v4z=1" target="_blank" rel="noreferrer"><Image src="/mamma/mammographie/verkalkungen/case-159211/r-mlo.png" alt="MLO-Mammographie der rechten Brust mit Hautverkalkungen" width={340} height={630}/></a><figcaption>Rechts MLO</figcaption></figure>
        <figure><a href="https://radiopaedia.org/cases/159211/studies/130479?lang=us#t=im&v1i=60684407&v1z=1&v2i=60684408&v2z=1&v3i=60684410&v3z=1&v4i=60684413&v4z=1" target="_blank" rel="noreferrer"><Image src="/mamma/mammographie/verkalkungen/case-159211/r-cc-detail.png" alt="Vergrößerung gruppierter Hautverkalkungen der rechten Brust" width={335} height={578}/></a><figcaption>Detailaufnahme</figcaption></figure>
    ...
    <article className={caseStyles.caseStudy}>
      ...
      <div className={caseStyles.caseGallery}>
        <figure><a href="https://radiopaedia.org/cases/72331/studies/82850?lang=us#t=im&v1i=51746512&v1z=1&v2i=51746513&v2z=1&v3i=51746514&v3z=1&v4i=51746515&v4z=1" target="_blank" rel="noreferrer"><Image src="/mamma/mammographie/verkalkungen/case-72331/r-mlo-1.png" alt="MLO-Mammographie mit vaskulären Verkalkungen" width={443} height={539}/></a><figcaption>MLO-Aufnahme</figcaption></figure>
...
`}</Lines><i style={{textDecoration:'underline'}}>Nicht verwechseln mit den deutlich feineren und irregulären fine linear calcifications.</i></p></div></article>
    ...
    <article className={`${caseStyles.caseStudy} ${caseStyles.exampleRod}`}>
      ...
      <div className={caseStyles.caseGallery}>
        <figure><a href="https://radiopaedia.org/cases/86379/studies/102416?lang=us#t=im&v1i=54305500&v1z=1&v2i=54305501&v2z=1&v3i=54305502&v3z=1&v4i=54305503&v4z=1" target="_blank" rel="noreferrer"><Image src="/mamma/mammographie/verkalkungen/case-86379/rod-1.png" alt="MLO-Aufnahme rechts mit Large-rod-like-Verkalkungen" width={491} height={720}/></a><figcaption>MLO rechts</figcaption></figure>
        <figure><a href="https://radiopaedia.org/cases/86379/studies/102416?lang=us#t=im&v1i=54305500&v1z=1&v2i=54305501&v2z=1&v3i=54305502&v3z=1&v4i=54305503&v4z=1" target="_blank" rel="noreferrer"><Image src="/mamma/mammographie/verkalkungen/case-86379/rod-2.png" alt="CC-Aufnahme rechts mit Large-rod-like-Verkalkungen" width={491} height={720}/></a><figcaption>CC rechts</figcaption></figure>
      </div>
  ...
  <Section {...GERMAN_SECTIONS[7]} title={GERMAN_SECTIONS[7].label.de}>
    ...
    <div className={caseStyles.modalityComparison}>
      <article>
        ...
        <dl>
          <div><dt>Sichtbarkeit</dt><dd>Makroverkalkungen erscheinen echogen, häufig mit dorsalem Schallschatten – etwa in Fibroadenomen, verkalkten Ölzysten oder bei Fettnekrose.<br/>Mikrokalk kann als feine echogene Foci sichtbar sein, besonders innerhalb einer Gewebeveränderung oder eines Ganges.</dd></div>
...
<Section {...SECTIONS[3]} title={tx(SECTIONS[3].label)}><div className={styles.riskArrow}>{tx(L('steigende Suspektheit','increasing suspicion','افزایش میزان شک'))} →</div><div className={`${styles.morphRail} ${caseStyles.morphologyScale}`}>{MORPH.map((x,i)=><article key={x.key}><span>{String(i+1).padStart(2,'0')}</span><h3>{tx(x.title)}</h3><small>{tx(x.risk)}</small><p>{tx(x.text)}</p><MorphologyImage type={x.key} lang={lang}/></article>)}</div></Section>
  <Section {...SECTIONS[4]} title={tx(SECTIONS[4].label)}><div className={caseStyles.morphologyArrow}><span>{tx(L('benigne','benign','خوش‌خیم'))}</span><b>{tx(L('suspekt','suspicious','مشکوک'))}</b></div><div className={`${styles.morphRail} ${caseStyles.morphologyScale} ${caseStyles.distributionRiskScale}`}>{DISTRIBUTION.map((x,i)=><article key={x.key}><span>{String(i+1).padStart(2,'0')}</span><h3>{tx(x.title)}</h3><p>{tx(x.text)}</p><DistributionImage type={x.key} lang={lang}/></article>)}</div><div className={styles.rule}><strong>{tx(L('Duktales Muster','Ductal pattern','الگوی داکتال'))}</strong><p>{tx(L('Linear und segmental sind besonders relevant, weil sie einem Gangsystem folgen können.','Linear and segmental patterns matter because they may follow a ductal system.','الگوهای خطی و سگمنتال مهم‌اند، زیرا ممکن است مسیر سیستم مجرایی را دنبال کنند.'))}</p></div></Section>
