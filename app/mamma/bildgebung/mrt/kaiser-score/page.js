...
function Section({ id, number, title, open, onToggle, children }) {
  return <section id={id} className={`${base.section} ${basics.section} ${styles.section}`}><button className={`${base.sectionHeader} ${basics.sectionHeader}`} type="button" onClick={() => onToggle(id)} aria-expanded={open}><span className={basics.sectionHeading}><span className={`${basics.sectionIcon} ${styles.sectionNumber}`}>{number}</span><h2>{title}</h2></span><span className={basics.sectionToggle}>{open ? '−' : '+'}</span></button>{open && <div className={`${base.sectionBody} ${basics.sectionBody} ${styles.sectionBody}`}>{children}</div>}</section>
}
  ...
  return <main className={`${base.page} ${basics.page} ${styles.page} ${lang === 'fa' ? styles.rtl : ''}`} dir={lang === 'fa' ? 'rtl' : 'ltr'} lang={lang}>
    <header className={base.header}>
      ...
      <div className={base.hero}>
        <div className={`${base.heroText} ${basics.heroText} ${styles.heroText}`}><span className={`${base.sourceBadge} ${basics.sourceBadge}`}>Dr. Zia · {tx(COPY.sourceNote)}</span><h1>{tx(COPY.title)}</h1><p>{tx(COPY.subtitle)}</p><div className={base.actions}><Link className={`${base.actionBtn} ${basics.actionBtn} ${styles.calculatorButton}`} href={withLang('/kaiser-score')}>{tx(COPY.calculator)} <span>↗</span></Link></div></div>
    ...
    <div className={base.layout}>
      <aside className={`${base.sidebar} ${basics.sidebar}`}><div className={base.sideTitle}>{tx(COPY.contents)}</div>{SECTIONS.map((section, index) => <button key={section.id} type="button" className={`${base.sideItem} ${basics.sideItem} ${activeId === section.id ? `${base.sideItemActive} ${basics.sideItemActive}` : ''}`} onClick={() => selectSection(section.id)}><span className={`${basics.sideNumber} ${styles.sideNumber}`}>{String(index + 1).padStart(2, '0')}</span><strong>{tx(section.label)}</strong></button>)}</aside>
      <div className={base.main}>
        <Section id="orientierung" number="01" title={tx(SECTIONS[0].label)} open={openId === 'orientierung'} onToggle={toggleSection}>
        ...
        <Section id="merkmale" number="02" title={tx(SECTIONS[1].label)} open={openId === 'merkmale'} onToggle={toggleSection}><FeatureAtlas lang={lang}/></Section>

        <Section id="trainer" number="03" title={tx(SECTIONS[2].label)} open={openId === 'trainer'} onToggle={toggleSection}><DecisionTrainer lang={lang} withLang={withLang}/></Section>

        <Section id="interpretation" number="04" title={tx(SECTIONS[3].label)} open={openId === 'interpretation'} onToggle={toggleSection}>
          <ScoreRail/>
        ...
        <Section id="faelle" number="05" title={tx(SECTIONS[4].label)} open={openId === 'faelle'} onToggle={toggleSection}>
        ...
        <Section id="fallstricke" number="06" title={tx(SECTIONS[5].label)} open={openId === 'fallstricke'} onToggle={toggleSection}>
          ...
          <div className={styles.takeHome}><span>{tx({ de: 'Merksatz', en: 'Take-home', fa: 'نکته کلیدی' })}</span><h3>Root → Curve → {tx({ de: 'relevantes drittes Merkmal', en: 'relevant third feature', fa: 'ویژگی سوم مرتبط' })}</h3><p>{tx({ de: 'Nicht alle fünf Merkmale werden addiert. Der Baum wählt in drei Entscheidungen den passenden Endknoten.', en: 'The five features are not added. The tree selects the appropriate terminal node in three decisions.', fa: 'پنج ویژگی با هم جمع نمی‌شوند؛ درخت با سه تصمیم، گره نهایی مناسب را انتخاب می‌کند.' })}</p></div>
