...
export default function MyokardinfarktMrtPage() {
  ...
  const sectionPanel = (section, children) => {
    ...
    return <section id={section.id} key={section.id} className={`${template.section} ${styles.card} ${open ? template.sectionOpen : ''}`}>
      <button type="button" className={`${template.sectionHeader} ${styles.cardHeader}`} onClick={() => selectSection(section.id)} aria-expanded={open} aria-controls={`${section.id}-panel`}>
        <span className={`${template.sectionIcon} ${styles.sectionIcon}`}><SectionIcon id={section.id} /></span><span><strong>{t(section.title)}</strong></span><span className={template.toggle} aria-hidden="true">{open ? '−' : '+'}</span>
      </button>
      <div id={`${section.id}-panel`} hidden={!open} className={`${template.sectionBody} ${styles.sectionBody}`}>
        {children}
        <button type="button" className={`${template.readButton} ${styles.sectionReadButton} ${done ? template.readButtonDone : ''}`} aria-pressed={done} onClick={() => toggleSectionRead(section.id)}><SectionIcon id="infarct" />{t(done ? copy.completedSection : copy.completeSection)}</button>
      </div>
    </section>
  }
  ...
  return <main className={`${template.page} ${styles.page}`} data-lesson-progress-managed="true" dir={current === 'fa' ? 'rtl' : 'ltr'} lang={current}>
    <header className={template.header}>
      ...
      <div className={template.hero}>
        ...
        <div className={`${template.heroFacts} ${styles.heroFacts}`}>
      ...
      <div className={`${template.progressBar} ${styles.progressBar}`}><div className={template.progressTrack}><i style={{ width: `${(readSections.size / lessonSections.length) * 100}%` }} /></div><span>{readSections.size} / {lessonSections.length} {t(copy.progress)}</span><div className={template.progressActions}><button type="button" className={`${template.lessonCompleteButton} ${styles.completeButton} ${isRead ? template.lessonCompleteButtonDone : ''}`} aria-pressed={isRead} onClick={toggleLessonRead}><SectionIcon id="infarct" />{t(isRead ? copy.readDone : copy.completeLesson)}</button><button type="button" className={template.continueButton} onClick={nextSection} disabled={activeIndex === lessonSections.length - 1}>{t(copy.continue)} <span aria-hidden="true">→</span></button></div></div>
      {authError && <p className={styles.authError}>{t(copy.auth)} <Link href={withLang('/sign-in')}>{t(copy.signIn)}</Link></p>}
    </header>
    <div className={template.layout}>
      <aside className={`${template.sidebar} ${styles.sidebar}`}><h2>{t(copy.path)}</h2><nav>{lessonSections.map(section => <button type="button" key={section.id} className={openId === section.id ? `${template.activeSideItem} ${styles.activeSideItem}` : ''} onClick={() => selectSection(section.id)} aria-current={openId === section.id ? 'location' : undefined} aria-label={`${t(copy.openSection)}: ${t(section.title)}`}><span className={template.sideIcon}><SectionIcon id={section.id} /></span><strong>{t(section.title)}</strong></button>)}</nav></aside>
    ...
    <div className={template.mobileLearningPath}>
      {mobilePathOpen && <section id="mrt-mobile-learning-path" className={`${template.mobilePathPanel} ${styles.mobilePathPanel}`} role="dialog" aria-label={t(copy.path)}><header><div><small>{t(copy.progress)}</small><strong>{readSections.size} / {lessonSections.length}</strong></div><button type="button" onClick={() => setMobilePathOpen(false)} aria-label={t(copy.close)}>×</button></header><nav>{lessonSections.map(section => <button type="button" key={section.id} className={openId === section.id ? `${template.mobilePathCurrent} ${styles.mobileActive}` : ''} onClick={() => selectSection(section.id)} aria-current={openId === section.id ? 'location' : undefined}><span className={template.mobilePathItemIcon}><SectionIcon id={section.id} /></span><span><strong>{t(section.title)}</strong><small>{t(section.title)}</small></span><i aria-hidden="true">{readSections.has(section.id) ? '✓' : ''}</i></button>)}</nav></section>}
