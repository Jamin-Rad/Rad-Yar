...
function Section({ section, index, lang, open, onToggle, children }) {
  ...
  return <section id={section.id} className={`${styles.section} ${open ? styles.sectionOpen : ''}`}>
    <button type="button" className={styles.sectionHeader} aria-expanded={open} aria-controls={`${section.id}-panel`} onClick={() => onToggle(section.id)}>
      ...
      <span><small>{String(index + 1).padStart(2, '0')}</small><strong>{pick(section.title)}</strong></span>
...
export default function LearningPagePrototype() {
  ...
  return <main className={styles.page} dir={lang === 'fa' ? 'rtl' : 'ltr'} lang={lang}>
    ...
    <header className={styles.header}>
      <nav className={styles.breadcrumb} aria-label="Breadcrumb"><Link href="/">RadYar</Link><span>/</span><span>Design Lab</span><span>/</span><strong>{pick(COPY.breadcrumb)}</strong></nav>
    ...
    <div className={styles.layout}>
      <aside className={styles.sidebar}>
        ...
        <nav>{SECTION_COPY.map((section, index) => <button type="button" key={section.id} className={openId === section.id ? styles.activeSideItem : ''} onClick={() => selectSection(section.id)} aria-current={openId === section.id ? 'location' : undefined} aria-label={`${pick(COPY.open)}: ${pick(section.short)}`}><span>{String(index + 1).padStart(2, '0')}</span><strong>{pick(section.short)}</strong></button>)}</nav>
      </aside>

      <article className={styles.lesson}>
        {SECTION_COPY.map((section, index) => <Section key={section.id} section={section} index={index} lang={lang} open={openId === section.id} onToggle={selectSection}>
    ...
    <footer className={styles.footer}><p>{pick(L('Design-Prototyp · Inhalte dienen der UI-Prüfung und ersetzen keine lokale SOP.', 'Design prototype · Content is for UI review and does not replace local protocols.', 'نمونه طراحی · محتوا برای بررسی رابط کاربری است و جایگزین پروتکل‌های محلی نیست.'))}</p></footer>
  </main>
}
