...
import { AK_LUXATION_LESSON } from '@/data/ak-luxation'
...
export default function AkLuxationPage() {
  const { lang } = useLanguage()
  const copy = UI[lang] || UI.de
  const c = v => L(v, lang)
  const rtl = lang === 'fa'
  const { isRead, toggleRead, authError } = useLessonReadStatus('ak-luxation')
  const [active, setActive] = useState('')
  const route = '/msk/trauma/ak-luxation'
  const withLang = href => lang === 'de' ? href : (href.includes('?') ? `${href}&lang=${lang}` : `${href}?lang=${lang}`)
  const rows = v => v.map(r => r.map(c))
  const D = AK_LUXATION_LESSON
        ...
        <div className={styles.hero}>
          <div className={styles.heroText}>
            ...
            <div className={styles.actions}>
              <Link className={styles.actionBtn} href={withLang(`/ueben/quiz?fach=msk&n=12&themen=ak-luxation&from=${encodeURIComponent(withLang(route))}`)}>🎯 {copy.mcq}</Link>
              <Link className={styles.actionBtn} href={withLang(`/flashcards/ak-luxation?from=${encodeURIComponent(withLang(route))}`)}>🧠 {copy.flash}</Link>
            </div>
          </div>
      ...
      <div className={styles.layout}>
        <aside className={styles.sidebar}>
          ...
          {D.sections.map(s => (
            <button type="button" key={s.id}
              ...
              style={s.emphasis ? { color: '#1d4ed8', border: '1px solid rgba(29,78,216,.48)', background: 'rgba(29,78,216,.12)', fontWeight: 950 } : undefined}>
              <span>{s.icon}</span>
        ...
        <div className={styles.main}>
          ...
          <Section id="anatomie" title={c(D.anatomie.title)} lead={c(D.anatomie.lead)}>
            ...
            <Callout label={c(D.keyLabel)}>{c(D.anatomie.key)}</Callout>
          </Section>

          {/* MECHANISMUS */}
          <Section id="mechanismus" title={c(D.mechanismus.title)} lead={c(D.mechanismus.lead)}>
            <Cards items={D.mechanismus.items} lang={lang} />
          </Section>

          {/* BILDGEBUNG */}
          <Section id="bildgebung" title={c(D.bildgebung.title)} lead={c(D.bildgebung.lead)}>
            <Table headers={D.bildgebung.headers.map(c)} rows={rows(D.bildgebung.rows)} />
            <div style={{ marginTop: '1rem', padding: '0.85rem 1rem', background: 'rgba(29,78,216,0.1)', border: '1px solid rgba(29,78,216,0.3)', borderRadius: '8px' }}>
              <strong style={{ fontSize: '0.9rem' }}>{c(D.bildgebung.ccdNote.title)}</strong>
              <p style={{ margin: '0.35rem 0 0', fontSize: '0.875rem', opacity: 0.9 }}>{c(D.bildgebung.ccdNote.text)}</p>
            </div>
            <CaseGallery
              title={c(D.bildgebung.caseTitle)}
              images={D.bildgebung.caseImages.map(img => ({ ...img, caption: c(img.caption) }))}
              radiopaediaUrl={D.bildgebung.radiopaediaUrl}
              copy={copy}
            />
          ...
          {/* ROCKWOOD */}
          <Section id="rockwood" title={c(D.rockwood.title)} lead={c(D.rockwood.lead)}>
            <SchemaImage src={D.rockwood.illustrationSrc} alt={c(D.rockwood.illustrationAlt)} copy={copy} fullWidth />
            <Table headers={D.rockwood.headers.map(c)} rows={rows(D.rockwood.rows)} />
            <div style={{ marginTop: '1rem', padding: '0.75rem 1rem', background: 'rgba(255,255,255,0.06)', borderRadius: '8px', fontSize: '0.85rem', opacity: 0.8 }}>
              <em>{c(D.rockwood.tossy)}</em>
            </div>
            <Callout label={c(D.keyLabel)}>{c(D.rockwood.key)}</Callout>
          </Section>

          {/* THERAPIE */}
          <Section id="therapie" title={c(D.therapie.title)} lead={c(D.therapie.lead)}>
            <Table headers={D.therapie.headers.map(c)} rows={rows(D.therapie.rows)} />
            <Callout label={c(D.keyLabel)}>{c(D.therapie.key)}</Callout>
          </Section>
