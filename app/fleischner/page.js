...
const COPY = {
  de: {
    brand: 'FLEISCHNER', steps: ['Anwendbarkeit', 'Rundherd', 'Messung', 'Risiko', 'Empfehlung'],
    intro: 'Management inzidenteller Lungenrundherde nach den Empfehlungen der Fleischner Society 2017.',
    startEyebrow: 'Sicher starten', startTitle: 'Darf Fleischner angewendet werden?', startText: 'Prüfen Sie zuerst die vier Voraussetzungen. Erst wenn die Leitlinie passt, beginnt die Rundherd-Einordnung.', startButton: 'Ja, starten',
    scopeTitles: { age: 'Alter ≥ 35 Jahre', incidental: 'Zufallsbefund', immune: 'Immunkompetent', cancer: 'Keine Tumorerkrankung' }, noHighRisk: 'Kein hohes Risiko', noHighRiskHint: 'Keine der abgefragten risikoerhöhenden Eigenschaften liegt vor. Alle Antworten auf Nein setzen.', findings: 'Befund', assessment: 'Beurteilung', scopeTitle: 'Ist die Leitlinie anwendbar?', scopeText: 'Die Fleischner-Empfehlungen gelten nur für inzidentell entdeckte Rundherde bei Erwachsenen.',
    ...
    noduleTitle: 'Rundherd einordnen', noduleText: 'Bei multiplen Herden wird der suspekteste – nicht zwingend der größte – Herd vermessen.',
    ...
    measurementTitle: 'Rundherd messen', measurementText: '',
    ...
    riskTitle: 'Risikoprofil', riskText: '',
    ...
    primaryRec: 'Empfohlenes Vorgehen', copy: 'Text kopieren', copied: 'Kopiert',
  ...
  en: {
    brand: 'FLEISCHNER', steps: ['Eligibility', 'Nodule', 'Measurement', 'Risk', 'Recommendation'],
    intro: 'Management of incidental pulmonary nodules according to the 2017 Fleischner Society recommendations.',
    startEyebrow: 'Start safely', startTitle: 'Can the Fleischner guideline be applied?', startText: 'Check the four eligibility criteria first. Nodule classification begins only when the guideline applies.', startButton: 'Yes, start',
    scopeTitles: { age: 'Age ≥ 35 years', incidental: 'Incidental finding', immune: 'Immunocompetent', cancer: 'No known cancer' }, noHighRisk: 'No high risk', noHighRiskHint: 'None of the listed risk-increasing features is present. Set all answers to No.', findings: 'Findings', assessment: 'Impression', scopeTitle: 'Do the guidelines apply?', scopeText: 'Fleischner recommendations apply only to incidentally detected nodules in adults.',
    ...
    noduleTitle: 'Classify the nodule', noduleText: 'With multiple nodules, measure the most suspicious nodule, which is not necessarily the largest.',
    ...
    measurementTitle: 'Measure the nodule', measurementText: '',
    ...
    riskTitle: 'Risk profile', riskText: '',
    ...
    primaryRec: 'Recommended management', copy: 'Copy text', copied: 'Copied',
...
function recommendation({ applicable, density, count, size, risk, solidComponent, lang }) {
  ...
  if (density === 'solid') {
    ...
    return { tone: 'watch', title: de ? 'CT nach 3–6 Monaten' : 'CT at 3–6 months', text: risk === 'high' ? (de ? 'Anschließend CT nach 18–24 Monaten.' : 'Then CT at 18–24 months.') : (de ? 'Anschließend CT nach 18–24 Monaten erwägen.' : 'Then consider CT at 18–24 months.'), context: de ? `Multiple solide Rundherde ${size <= 8 ? '6–8 mm' : '> 8 mm'} · ${risk === 'high' ? 'höheres' : 'niedrigeres'} Risiko.` : `Multiple solid nodules ${size <= 8 ? '6–8 mm' : '> 8 mm'} · ${risk === 'high' ? 'higher' : 'lower'} risk.` }
  }
  if (count === 'multiple') {
...
function RiskQuestion({ label, value, yes, no, onChange }) {
  return <div className={styles.riskQuestion}><strong>{label}</strong><div><button type="button" className={value === false ? styles.riskNoActive : ''} onClick={() => onChange(false)} aria-pressed={value === false}>{value === false ? '✓ ' : ''}{no}</button><button type="button" className={value === true ? styles.riskYesActive : ''} onClick={() => onChange(true)} aria-pressed={value === true}>{value === true ? '✓ ' : ''}{yes}</button></div></div>
}
...
export default function FleischnerPage() {
  ...
  const analytics = useCalculatorAnalytics('fleischner')
  ...
  const stepIds = !applicable ? [0, 4] : needsRiskAssessment ? [0, 1, 2, 3, 4] : [0, 1, 2, 4]
  ...
  const densityLabel = activeLang === 'de'
    ? density === 'solid' ? 'solider Lungenrundherd' : density === 'ggo' ? 'reiner Milchglasrundherd' : 'part-solider Lungenrundherd'
    : density === 'solid' ? 'solid pulmonary nodule' : density === 'ggo' ? 'pure ground-glass nodule' : 'part-solid pulmonary nodule'
  const densityPlural = activeLang === 'de'
    ? density === 'solid' ? 'solide Lungenrundherde' : density === 'ggo' ? 'reine Milchglasrundherde' : 'part-solide Lungenrundherde'
    : density === 'solid' ? 'solid pulmonary nodules' : density === 'ggo' ? 'pure ground-glass nodules' : 'part-solid pulmonary nodules'
  const findingsText = !applicable ? '' : activeLang === 'de'
    ? count === 'single'
      ? `Inzidenteller solitärer ${densityLabel} mit einem mittleren Durchmesser von ${meanSize} mm${needsSolidComponent && solidSize > 0 ? ` und einer soliden Komponente von ${solidSize} mm` : ''}.`
      : `Inzidentelle multiple ${densityPlural}; der suspekteste Herd misst im Mittel ${meanSize} mm.`
    : count === 'single'
      ? `Incidental single ${densityLabel} with a mean diameter of ${meanSize} mm${needsSolidComponent && solidSize > 0 ? ` and a ${solidSize} mm solid component` : ''}.`
      : `Incidental multiple ${densityPlural}; the most suspicious nodule has a mean diameter of ${meanSize} mm.`
  const riskAssessmentText = !needsRiskAssessment || !riskComplete ? '' : activeLang === 'de'
    ? risk === 'high'
      ? 'Aufgrund der angegebenen Risikomerkmale ist von einem höheren klinischen Risikoprofil auszugehen. '
      : positiveRiskKeys.length
        ? 'Die angegebenen Einzelmerkmale ergeben insgesamt kein hohes klinisches Risikoprofil. '
        : 'Es liegt kein erhöhtes klinisches Risikoprofil vor. '
    : risk === 'high'
      ? 'The reported features indicate a higher clinical risk profile. '
      : positiveRiskKeys.length
        ? 'The reported individual features do not amount to a high clinical risk profile. '
        : 'No elevated clinical risk profile is present. '
  const assessmentText = applicable
    ? `${riskAssessmentText}${activeLang === 'de' ? 'Empfehlung gemäß Fleischner Society 2017' : 'Recommendation according to the 2017 Fleischner Society guideline'}: ${result.title}. ${result.text}`
    : result.text
  const reportText = applicable ? `${ui.findings}:\n${findingsText}\n\n${ui.assessment}:\n${assessmentText}` : `${ui.assessment}:\n${assessmentText}`
  ...
  const canContinue = step === 0 ? true : step === 1 ? Boolean(density && count) : step === 2 ? Boolean(meanSize && axesValid && solidComponentValid) : step === 3 ? riskComplete : true
  const next = () => { if (!canContinue) { setError(step === 2 && meanSize && !axesValid ? ui.axisError : step === 2 && meanSize && !solidComponentValid ? ui.solidError : ui.required); return }; analytics.start(); setError(''); const target = !applicable ? 4 : step === 2 && !needsRiskAssessment ? 4 : Math.min(step + 1, 4); setCompleted(value => Math.max(value, target)); setStep(target); if (target === 4) analytics.complete() }
  const back = () => { setError(''); const target = !applicable && step === 4 ? 0 : step === 4 && !needsRiskAssessment ? 2 : Math.max(0, step - 1); setStep(target); setCompleted(target) }
  const restart = () => { analytics.restart(); setStep(0); setCompleted(0); setEligibility({ age: true, incidental: true, immune: true, cancer: true }); setDensity('solid'); setCount('single'); setLongAxis(''); setShortAxis(''); setLongAxisManuallyAdjusted(false); setSolidComponent(''); setRiskAnswers({ smoking: null, morphology: null, upperLobe: null, age: null, lungDisease: null, background: null }); setError(''); setCopied(false); setShareOpen(false); setLinkCopied(false) }
  ...
  const copyLink = async () => { try { await navigator.clipboard.writeText(FLEISCHNER_URL); analytics.track('copy_link'); setLinkCopied(true); window.setTimeout(() => setLinkCopied(false), 1800) } catch {} }
  ...
  return <main className={styles.page} lang={activeLang} dir="ltr">
    ...
    <div className={styles.shell}>
      ...
      <section className={styles.workspace}>
        <Stepper step={stepIds.indexOf(step)} completed={stepIds.filter(id => id <= completed).length - 1} labels={stepIds.map(id => ui.steps[id])} onStep={(index) => { const target = stepIds[index]; setError(''); setStep(target); setCompleted(target) }}/>
        <div className={styles.viewport} key={step}>
          {step === 0 ? <div className={styles.step}><header><h2>{ui.scopeTitle}</h2><p>{ui.scopeText}</p></header><div className={styles.scopeGrid}>{Object.keys(eligibility).map(key => <button type="button" key={key} className={eligibility[key] ? styles.checked : ''} onClick={() => setEligibility(values => ({ ...values, [key]: !values[key] }))} aria-pressed={eligibility[key]}><span>{eligibility[key] ? '✓' : '×'}</span><div><strong>{ui.scopeTitles[key]}</strong><p>{ui[key]}</p></div></button>)}</div><div className={`${styles.note} ${!applicable ? styles.warning : ''}`}><i>i</i><p>{ui.scopeInfo}</p></div></div> : null}
          ...
          {step === 2 ? <div className={styles.step}><header><h2>{ui.measurementTitle}</h2>{ui.measurementText ? <p>{ui.measurementText}</p> : null}</header>{count === 'multiple' ? <div className={styles.leadNote}><span>◎</span><p><strong>{ui.suspiciousLead}</strong>{ui.noduleText}</p></div> : null}<div className={styles.measureGrid}><Measurement label={ui.shortAxis} value={shortAxis} onChange={changeShortAxis}/><Measurement label={ui.longAxis} value={longAxis} onChange={changeLongAxis} min={Number(shortAxis) || 1}/>{needsSolidComponent ? <Measurement label={ui.solidPart} value={solidComponent} onChange={setSolidComponent} max={Math.max(meanSize, 6)}/> : null}</div><div className={styles.meanResult}><span>{ui.calculatedMean}<small>{ui.formula}</small></span><strong>{meanSize ?? '—'}<small>mm</small></strong></div></div> : null}
          {step === 3 ? <div className={`${styles.step} ${styles.riskStep}`}><header><h2>{ui.riskTitle}</h2>{ui.riskText ? <p>{ui.riskText}</p> : null}</header><section className={styles.riskAssessment}><button type="button" className={styles.noHighRisk} aria-pressed={riskComplete && positiveRiskKeys.length === 0} onClick={() => { setRiskAnswers(Object.fromEntries(Object.keys(riskAnswers).map(key => [key, false]))); setError('') }}><span>{riskComplete && positiveRiskKeys.length === 0 ? '✓' : '○'}</span><div><strong>{ui.noHighRisk}</strong><p>{ui.noHighRiskHint}</p></div></button><div className={styles.riskQuestions}>{Object.keys(riskAnswers).map(key => <RiskQuestion key={key} label={ui.riskQuestions[key]} value={riskAnswers[key]} yes={ui.yes} no={ui.no} onChange={value => changeRiskAnswer(key, value)}/>)}</div>{riskComplete ? <div className={styles.riskOutcome} data-risk={risk}><span>{risk === 'high' ? '↑' : '↓'}</span><div><small>{ui.riskAssessment}</small><strong>{risk === 'high' ? ui.high : ui.low}</strong><p>{risk === 'high' ? ui.highHint : ui.lowHint} {ui.riskMethod}</p></div></div> : <p className={styles.riskPending}>{ui.riskPending}</p>}</section></div> : null}
          {step === 4 ? <div className={`${styles.step} ${styles.resultStep}`}><header><h2>{applicable ? ui.resultTitle : ui.notApplicable}</h2><p>{applicable ? ui.resultText : ui.notApplicableText}</p></header>{resultReady ? <><article className={styles.recommendation} data-tone={result.tone}><span className={styles.recommendationIcon}>{result.tone === 'quiet' ? '✓' : result.tone === 'stop' ? '!' : result.tone === 'action' ? '◆' : '◷'}</span><small>{ui.primaryRec}</small><h3>{result.title}</h3><p>{result.text}</p></article><div className={styles.report}><div>{applicable ? <section><h4>{ui.findings}</h4><p>{findingsText}</p></section> : null}<section><h4>{ui.assessment}</h4><p>{assessmentText}</p></section></div><button type="button" onClick={copyReport}>{copied ? '✓' : '⧉'} {copied ? ui.copied : ui.copy}</button></div></> : null}<section className={`${styles.shareCard} ${shareOpen ? styles.shareCardOpen : ''}`}><button type="button" className={styles.shareToggle} onClick={() => { if (!shareOpen) analytics.track('recommend_open'); setShareOpen(value => !value) }} aria-expanded={shareOpen}><span className={styles.shareIcon}><ShareIcon/></span><span><strong>{ui.recommend}</strong><small>{ui.recommendHint}</small></span><i>{shareOpen ? '−' : '+'}</i></button>{shareOpen ? <div className={styles.sharePanel}><header><strong>{ui.shareTitle}</strong><p>{ui.shareText}</p></header><div className={styles.shareBody}><div className={styles.qrFrame}><QRCodeSVG value={FLEISCHNER_URL} size={184} level="H" bgColor="#effff8" fgColor="#06100f" marginSize={2} imageSettings={{ src: '/fleischner/fleischner-icon-192.png', width: 42, height: 42, excavate: true }}/></div><div className={styles.shareDetails}><span>{ui.scanLabel}</span><strong>rad-yar.com/fleischner</strong><div className={styles.shareActions}><a href={`https://wa.me/?text=${whatsAppText}`} target="_blank" rel="noreferrer" onClick={() => analytics.track('whatsapp_click')}><WhatsAppIcon/>{ui.whatsapp}</a><button type="button" onClick={copyLink}><span>{linkCopied ? '✓' : '⧉'}</span>{linkCopied ? ui.linkCopied : ui.copyLink}</button></div></div></div></div> : null}</section></div> : null}
        </div>
        {error ? <p className={styles.error} role="alert">{error}</p> : null}
        <footer className={`${styles.actions} ${step === 0 ? styles.scopeActions : ''}`}>{step > 0 ? <button type="button" className={styles.back} onClick={back}>← {ui.back}</button> : <span/>}{step < 4 ? <button type="button" className={styles.next} onClick={next} aria-disabled={!canContinue}>{step === 0 && applicable ? ui.startButton : ui.next} →</button> : <button type="button" className={styles.next} onClick={restart}>{ui.restart} ↻</button>}</footer>
      </section>
    </div>
