...
export default function FleischnerPage() {
  ...
  const next = () => { if (!canContinue) { setError(step === 2 && meanSize && !axesValid ? ui.axisError : step === 2 && meanSize && !solidComponentValid ? ui.solidError : ui.required); return }; setError(''); const target = !applicable ? 4 : step === 2 && !needsRiskAssessment ? 4 : Math.min(step + 1, 4); setCompleted(value => Math.max(value, target)); setStep(target) }
