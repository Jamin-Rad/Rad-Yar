...
function RecipientDetail({ recipient, state }) {
  ...
  return <section className={styles.reportSection}>
    ...
    <div className={styles.summaryLine}><span>یوروی دریافت‌شده: <strong>{formatEuro(ledger.confirmedEuro)}</strong></span><span>در انتظار دریافت، شامل قول کمک: <strong>{formatEuro(ledger.pendingEuro)}</strong></span><span>کل ثبت‌شده: <strong>{formatEuro(ledger.euro)}</strong></span>{ledger.promisedEuro ? <span>قول کمکِ در انتظار: <strong>{formatEuro(ledger.promisedEuro)}</strong></span> : null}<span>معادل کمک ثبت‌شده، تومان: <strong>{formatToman(ledger.toman)}</strong></span><span>ماندهٔ واریز به ستاد دیه، شامل قول کمک: <strong>{formatToman(ledger.owedToman)}</strong></span></div>
      ...
      {['overview', 'recipients', 'recipient'].includes(type) ? <>
        <section className={styles.reportSection}><h2>گیرنده‌ها و ماندهٔ واریز به ستاد دیه</h2><ReportTable headers={['گیرنده', 'یوروی دریافت‌شده', 'یوروی در انتظار دریافت', 'معادل تومان', 'باید به ستاد دیه واریز کند', 'تعداد زندانی']}>
          {(type === 'recipient' ? [recipient] : recipients).map(row => {
            ...
            return <tr key={row.id || 'unassigned'}><td>{row.name}</td><td>{formatEuro(ledger.confirmedEuro)}</td><td>{formatEuro(ledger.pendingEuro)}{ledger.promisedEuro ? <><br />شامل قول کمک: {formatEuro(ledger.promisedEuro)}</> : null}</td><td>{formatToman(ledger.toman)}</td><td>{formatToman(ledger.owedToman)}{ledger.promisedEuro ? <><br />شامل قول کمک</> : null}</td><td>{ledger.cases.length}</td></tr>
          })}
        </ReportTable></section>
