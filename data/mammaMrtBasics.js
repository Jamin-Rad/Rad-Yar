...
const QUESTION_CONTENT = [
  ...
  },
  {
    ...
    options: [
      L('Enhancement deutlicher sichtbar zu machen', 'To make enhancement more conspicuous', 'واضح‌تر کردن نواحی Enhancement'),
      ...
      L('Die T2-Sequenz zu ersetzen', 'To replace the T2 sequence', 'جایگزین‌کردن T2 Sequence'),
    ],
    correct: 'A',
    explanation: L('Das Präkontrastbild wird vom Postkontrastbild abgezogen. Dadurch lässt sich echtes Enhancement leichter erkennen.', 'The pre-contrast image is subtracted from the post-contrast image, making true enhancement easier to detect.', 'تصویر قبل از تزریق از تصویر پس از تزریق کم می‌شود؛ در نتیجه Enhancement واقعی واضح‌تر دیده می‌شود.'),
  },
  {
    id: 'mip',
    ...
    options: [
      ...
      L('Sie ersetzt alle Einzelsequenzen.', 'It replaces all source sequences.', 'جایگزین تمام Source Sequenceها می‌شود.'),
...
const FLASHCARD_CONTENT = [
  ['Grundprinzip', 'Warum ist die Mamma-MRT besonders sensitiv?', 'Sie zeigt Tumorvaskularisation und Kontrastmittelaufnahme.', 'Die Spezifität bleibt begrenzt, weil auch benigne Veränderungen enhancen.', 'Principle', 'Why is breast MRI particularly sensitive?', 'It depicts tumour vascularity and contrast enhancement.', 'Specificity remains limited because benign findings can also enhance.', 'اصل', 'چرا MRI پستان حساسیت بالایی دارد؟', 'عروق تومور و الگوی Enhancement پس از تزریق را نشان می‌دهد.', 'اختصاصیت محدود است، زیرا ضایعات خوش‌خیم نیز ممکن است Enhancement نشان دهند.'],
  ['Indikation', 'Nenne eine zentrale Screening-Indikation.', 'Deutlich erhöhtes Lebenszeitrisiko für ein Mammakarzinom.', 'Die MRT ergänzt Mammographie und Sonographie gezielt.', 'Indication', 'Name a key screening indication.', 'A markedly increased lifetime risk of breast cancer.', 'MRI is used as a targeted adjunct to mammography and ultrasound.', 'موارد کاربرد', 'یکی از موارد اصلی استفاده از MRI در غربالگری چیست؟', 'افزایش واضح خطر مادام‌العمر ابتلا به سرطان پستان.', 'MRI به‌صورت هدفمند در کنار ماموگرافی و سونوگرافی استفاده می‌شود.'],
  ['Protokoll', 'Wozu dient T1 vor Kontrastmittel?', 'Als Ausgangsbasis und für die spätere Subtraktion.', 'Sie hilft zudem, Fett, Blutprodukte und vorbestehend hohes T1-Signal zu erkennen.', 'Protocol', 'What is pre-contrast T1 used for?', 'As a baseline and for later subtraction.', 'It also helps identify fat, blood products and pre-existing high T1 signal.', 'پروتکل', 'T1 قبل از تزریق چه کاربردی دارد؟', 'تصویر پایه را فراهم می‌کند و برای Subtraction لازم است.', 'همچنین در تشخیص چربی، محصولات خونی و سیگنال بالای T1 که پیش از تزریق وجود داشته کمک‌کننده است.'],
  ['Subtraktion', 'Was zeigt die Subtraktion?', 'Sie macht Kontrastmittel-Enhancement deutlich besser sichtbar.', 'Das Präkontrastbild wird vereinfacht vom Postkontrastbild abgezogen.', 'Subtraction', 'What does subtraction show?', 'It makes contrast enhancement much more conspicuous.', 'The pre-contrast image is subtracted from the post-contrast image.', 'Subtraction', 'Subtraction چه چیزی را نشان می‌دهد؟', 'نواحی Enhancement را بسیار واضح‌تر می‌کند.', 'تصویر قبل از تزریق از تصویر پس از تزریق کم می‌شود.'],
  ...
  ['Diffusion', 'Bedeutet ein niedriger ADC automatisch Krebs?', 'Nein.', 'DWI/ADC ist nur ein zusätzlicher Baustein und muss mit den übrigen Sequenzen korreliert werden.', 'Diffusion', 'Does a low ADC automatically mean cancer?', 'No.', 'DWI/ADC is an adjunct and must be correlated with the other sequences.', 'Diffusion', 'آیا ADC پایین به‌تنهایی به معنی سرطان است؟', 'خیر.', 'DWI و ADC فقط بخشی از ارزیابی‌اند و باید با سایر MRI Sequenceها تطبیق داده شوند.'],
